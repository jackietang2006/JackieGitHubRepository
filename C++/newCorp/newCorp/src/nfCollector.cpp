...
int main(int argc, char **argv)
{
   ...
   */
  if (argc != 2) {
    error("usage: nfCollector <binding port>");
  }
  portno = atoi(argv[1]);
  ...
  //Start NF AnalyzerThread
  NfAnalyzerThread::singleton()->Start();

  //Start NF MonitorThread
  NfAnalyzerThread::singleton()->sharedMemoryKey(1111);
  NfMonitorThread::singleton()->setThreadToBeMonitored(NfAnalyzerThread::singleton());
  ...
  while (1) {
    ...
    NfAnalyzerThread::singleton()->enqueMsg(packet, udpDataGramLen, clientaddr.sin_addr.s_addr);
