...
void netFlowExport(char* inDir, char* inFileName)
{
  ...
  int udpHdrLen = 0, maxUdpHdrLen = 0, sumOfUdpHdrLen = 0;

  char fullName[MAX_CHARS_PER_LINE];
  char errbuf[PCAP_ERRBUF_SIZE];
  const u_char *packet;
  u_char rawUdpDG[BUFFERSIZE];
  int rawUdpDG_len;
  ...
  RawUdp aRawUdp(servaddr.sin_addr.s_addr, servaddr.sin_port);
  ...
  while (0 != (packet = pcap_next(pcapFB, &header)))
  {
    ...
    //ETH frame
    const u_char *pkt_ptr = (u_char *)packet;
    int ether_offset = 0;
    ...
    //Good NF packets
    totalNfRecords ++;
    sumOfUdpHdrLen += udpHdrLen;
    maxUdpHdrLen = (maxUdpHdrLen < udpHdrLen) ? udpHdrLen : maxUdpHdrLen;

    aRawUdp.buildRawUdpDataGram(ip_hdr->saddr, udp_hdr->source, pkt_ptr, ntohs(udp_hdr->len) - 8, rawUdpDG, rawUdpDG_len, csum);
    sendto(sockfd, rawUdpDG, ntohs(rawUdpDG_len), 0, (struct sockaddr *)&servaddr, sizeof(servaddr));
  ...
  fprintf(inputUdpPacketsFB, "Max UDP HDR Len: %d --- Average UDP HDR Len: %d\n", maxUdpHdrLen, sumOfUdpHdrLen/totalNfRecords);
  pcap_close(pcapFB);
  pcapFB=NULL;
}
